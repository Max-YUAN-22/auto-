#!/usr/bin/env python3
"""
Fixed Real Multi-Agent System Benchmarking
修复的真实多智能体系统基准测试

This script fixes the issues with LangChain, CrewAI, and AutoGen testing
and provides comprehensive real-world benchmarking.
这个脚本修复了LangChain、CrewAI和AutoGen测试的问题，提供全面的真实世界基准测试。
"""
...
class FixedMultiAgentBenchmark:
    """Fixed multi-agent system benchmarking"""
    ...
    async def run_focused_benchmark(self):
        """Run focused benchmark on smaller scale for debugging"""
        logger.info("Starting focused multi-agent system benchmark...")
        
        # Test configurations - smaller scale for debugging
        agent_counts = [1, 5, 10]
        frameworks_to_test = ['our_dsl', 'langchain', 'crewai', 'autogen']
        scenarios = ['traffic_management']  # Focus on one scenario first
        ...
        with open('results/fixed_multi_agent_benchmark.json', 'w') as f:
            json.dump(self.results, f, indent=2)
        
        logger.info("Focused benchmark completed!")
        return all_results
    
    def print_results(self):
        """Print benchmark results"""
        print("\n" + "="*80)
        print("FIXED MULTI-AGENT SYSTEM BENCHMARK RESULTS")
        print("="*80)
...
async def main():
    """Main function"""
    print("Fixed Real Multi-Agent System Benchmarking Framework")
    print("=" * 60)
    
    # Create benchmarker
    benchmarker = FixedMultiAgentBenchmark()
    
    # Run focused benchmark
    results = await benchmarker.run_focused_benchmark()
    
    # Print results
    benchmarker.print_results()
    
    print(f"\nResults saved to: results/fixed_multi_agent_benchmark.json")
